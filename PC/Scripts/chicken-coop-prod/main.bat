...
set host=chicken-coop
set user=farduino
set cert="c:\code\Certificates\Chicken Coop\Chicken Coop Private.ppk"

rem name of the WinSCP profile that we will use - use certificates
set scpprofile="ChickenCoop"

if "%1" == "" goto Args

rem scp-file to the Raspberry Pi in the Chicken Coop, ready for a firmware update
winscp /command "option batch abort" "option confirm off" "open %scpprofile%" "put %1 /firmware/main/firmware.hex" "exit"
...
plink %user%@%host% -i %CERT% /firmware/main/upload %2 %3 %4 %5 %6 %7 %8 %9
if %ERRORLEVEL% GTR 0 goto FAILED
